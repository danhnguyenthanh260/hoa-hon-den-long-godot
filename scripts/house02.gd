...
static func build(parent: Node3D, pos: Vector3, yrot: float, age: float = 0.15, out: Dictionary = {}) -> Node3D:
	...
	for s in [-1.0, 1.0]:
		...
		Build.box(a, Vector3(0.35, sill - H_BASE, o_w), Vector3(0.175, (H_BASE + sill) / 2.0, s * o_c), m["plaster"])
		Build.box(a, Vector3(0.35, Y1 - head, o_w), Vector3(0.175, (head + Y1) / 2.0, s * o_c), m["plaster"])
		...
		Build.breeze_panel(a, Vector3(-0.01, (sill + head) / 2.0, s * o_c), 3, 3, (o_w - 0.16) / 3.0)
		for ed in [-1.0, 1.0]:
			Build.box(a, Vector3(0.07, 0.07, o_w + 0.2), Vector3(-0.025, (sill if ed < 0 else head) + ed * 0.02, s * o_c), m["trim"])
			Build.box(a, Vector3(0.07, head - sill + 0.1, 0.07), Vector3(-0.025, (sill + head) / 2.0, s * (o_c + ed * (o_w / 2.0 + 0.02))), m["trim"])
		Build.box(a, Vector3(0.42, 0.07, o_w + 0.2), Vector3(0.0, sill - 0.07, s * o_c), m["trim"])
		# ánh đèn ấm trong nhà hắt qua khe hoa — đặt TRONG bề dày lỗ tường (trước mặt khối đặc x=0.35)
		var glow := Build.box(a, Vector3(0.02, o_w - 0.06, o_w - 0.06), Vector3(0.3, (sill + head) / 2.0, s * o_c), Build.emis(Color(1.0, 0.75, 0.45), Color(1.0, 0.55, 0.22), 0.5))
	...
	# hai cánh đóng kín — rèm/nội thất là chuyện BÊN TRONG, không bày ra mặt phố
	for s in [-1.0, 1.0]:
	...
	Build.box(a, Vector3(0.06, 0.24, W - 0.02), Vector3(-0.045, Y1 - 0.1, 0), m["frame"])
