...
static func build(parent: Node3D, pos: Vector3, yrot: float, age: float = 0.5, out: Dictionary = {}) -> Node3D:
	...
	for sz in [-1.0, 1.0]:
		# tâm ±1.85, khung 0.9: khe cửa↔khung 0.22m, khung↔trụ 0.15m (Phase 0)
		var wz2: float = sz * 1.85
		Build.box(a, Vector3(0.08, 1.2, 0.9), Vector3(-0.03, H_BASE + 1.55, wz2), m["frame"])
		var leaf := Build.box(a, Vector3(0.06, 1.1, 0.8), Vector3(-0.06, H_BASE + 1.55, wz2), m["shutter"])
		wins.append(leaf)
		for sl in range(6):
			var slat := Build.box(a, Vector3(0.03, 0.045, 0.7), Vector3(-0.09, H_BASE + 1.12 + sl * 0.17, wz2), m["frame"])
			slat.rotation.z = 0.55
		Build.box(a, Vector3(0.3, 0.07, 0.9), Vector3(-0.1, H_BASE + 0.92, wz2), m["trim"])
	out["windows"] = wins
	...
	var ridge_y := Parts.roof_amduong(a, W, D, Y1 + 0.05, m)
