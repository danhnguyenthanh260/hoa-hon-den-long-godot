...
func _test_choice_and_endings() -> void:
	...
	_check(state.ending_key() == "loop", "pre-C5 evidence should not unlock the release ending")
	state.add_evidence("c5_water_refuses_body")
	state.add_evidence("c5_blank_tablet_accepts_ash")
	state.add_evidence("c5_lantern_is_anchor")
