...
def run_playthrough(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        interact_at(hwnd, 5.85, -116.95, "c5 inspect foundation socket")
        interact_at(hwnd, -5.75, -118.55, "c5 take foundation stone")
        interact_at(hwnd, 5.85, -116.95, "c5 restore foundation")
        interact_at(hwnd, 5.85, -116.95, "c5 take tho")
        wait_until("Tho color unlocked", lambda s: s.get("current_color") == "tho", timeout=5.0)
        pyautogui.press("1")
        interact_at(hwnd, -6.15, -116.65, "c5 offer hoa")
        pyautogui.press("2")
        interact_at(hwnd, -6.15, -119.75, "c5 offer thuy")
        pyautogui.press("3")
        interact_at(hwnd, -5.65, -123.0, "c5 offer moc")
        pyautogui.press("4")
        interact_at(hwnd, 5.65, -123.0, "c5 offer kim")
        pyautogui.press("5")
        interact_at(hwnd, 6.15, -119.75, "c5 offer tho")
